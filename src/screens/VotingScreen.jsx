...
function VotingScreen() {
  ...
  if (currentPlayerIndex === null || currentPlayerIndex >= gameState.players.length) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmitVote} className="player-list-vote">
        {otherPlayers.map(player => (
          <label key={player.id}>
            <input
              ...
              name="vote"
              value={player.id}
              checked={selectedVote === String(player.id)}
        ...
        <button type="submit" disabled={!selectedVote}>
          Cast Vote for {gameState.players.find(p => p.id === parseInt(selectedVote))?.name || '...'}
        </button>
      </form>
    </div>
  );
