...
function NightPhaseScreen() {
  const { gameState, performNightAction, advanceNightAction, NIGHT_ACTION_ORDER } = useGame();
  ...
  if (currentRoleIndex < 0 || currentRoleIndex >= NIGHT_ACTION_ORDER.length) {
  ...
  const renderActionComponent = () => {
    if (gameState.isLoading) {
      return <LoadingScreen message={`Processing ${activeRoleName} action...`} />;
    }
    ...
    if (actionSubmitted && gameState.actionResult) {
      ...
      const resultToShow = gameState.actionResult.results?.map(item => item?.roleName || item?.name || item) || 'OK';
      return (
        <div>
          ...
          <button onClick={handleSkipOrContinue}>Next Role</button>
        </div>
      );
    }
    ...
    if (playersWithActiveRole.length === 0) {
      return (
        <div>
          <p>No players started as {activeRoleName}.</p>
          <button onClick={handleSkipOrContinue}>Next Role</button>
        </div>
      );
    }
    ...
    switch (activeRoleName) {
      case 'Seer':
        return <SeerAction players={gameState.players} groundCards={gameState.groundCards} actingPlayerId={actingPlayer.id} onSubmit={handleActionSubmit} />;
      case 'Robber':
        return <RobberAction players={gameState.players} actingPlayerId={actingPlayer.id} onSubmit={handleActionSubmit} />;
      case 'TroubleMaker':
        return <TroublemakerAction players={gameState.players} actingPlayerId={actingPlayer.id} onSubmit={handleActionSubmit} />;
      case 'Drunk':
        return <DrunkAction groundCards={gameState.groundCards} actingPlayerId={actingPlayer.id} onSubmit={handleActionSubmit} />;
      // Add cases for other roles like Doppelganger if they have interactive actions
      default:
        return (
          <div>
            <p>{activeRoleName} action UI not implemented yet.</p>
            <button onClick={handleSkipOrContinue}>Skip (Dev)</button>
          </div>
        );
    }
  };
  ...
  return (
    <div>
      ...
      <p>Player(s) with the role {activeRoleName}, perform your action.</p>
